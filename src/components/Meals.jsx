...
const Meals = () => {
  ...
  const {meals, loading, setShowModal, setMealId} = useGlobalContext();
  ...
  return (
    <div className="container mx-auto m-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 justify-items-center ">
      {meals.map(item => {
          ...
          return( 
            <div key={item.idMeal} className="relative bg-white p-3 m-5 drop-shadow-xl rounded-lg bg-[#f3edde] transition  ease-in-out duration-300 hover:-translate-y-1 hover:scale-105 duration-300 cursor-pointer opacity-95 hover:opacity-100" onClick={()=>{
                setMealId(item)
                setShowModal(true)
              }}> 
              <img src={item.strMealThumb}/>
              <div  className="absolute text-2xl fill-zinc-200 top-0 right-0 cursor-pointer ease-in-out duration-300 hover:-translate-y-1 hover:scale-110 duration-300" onClick={()=>{ console.log("added to Fav")}}>{heartFillSVG}</div>
